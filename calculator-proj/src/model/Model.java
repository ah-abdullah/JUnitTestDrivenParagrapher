...
public class Model {
	...
	private String doubleToString( double x, int base, int prec, boolean showDecimal ) {
		...
		if( x == Double.NEGATIVE_INFINITY ) return "- oo" ; // there was a bug here, fixed by changing Double.POSITIVE_INFINITY to Double.NEGATIVE_INFINITY
	...
	private void doPendingOperation() {
		...
		try {
			switch( op ) {
				...
				case DIVIDE: x /= y ; break ; // there was a bug here, fixed by changing x*=y to x/=y
